// ============================================================
// BUG-008 — INVESTIGAÇÃO-003
// Página migrada de Client Component (padrão antigo) para
// Server Component com SSR seguro:
//   - createSupabaseServerClient (client autenticado, respeita RLS)
//   - Validação de sessão → redirect('/painel/login')
//   - Validação de role integrador + perfil ativo
//   - Propriedade do projeto via user_id
// Layout, cálculos e comportamento visual preservados.
// ============================================================
...
export default async function PropostaDigital({
  params,
}: {
  ...
  // 1. Validação de ID numérico (mantém a tela padrão se inválido)
  if (!projectId || isNaN(Number(projectId))) {
    return (
      <div className="text-center p-12 text-slate-400 bg-slate-950 h-screen">
  ...
  // 4. Busca do projeto no servidor, com validação de propriedade
  ...
  Number(projeto.preco_venda) || (kwp * 3800);

const paybackAnos =
  investimentoReal > 0
    ? investimentoReal / economiaAnual
    : 0;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-orange-500/30">
      ...
      <div className="bg-slate-900 border-b border-slate-800 p-4 sticky top-0 z-50 flex justify-between items-center print:hidden">
        <Link
          href={`/painel/projetos/${projectId}`}
      ...
      <main className="max-w-4xl mx-auto p-6 md:p-12 space-y-12 print:p-0 print:bg-white print:text-slate-900">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-800 print:border-slate-300 pb-8">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h1 className="text-2xl font-black text-white tracking-tight print:text-slate-900">
                EnerTrack Solar
              </h1>
            </div>
            <p className="text-slate-400 print:text-slate-600">
              Proposta Tecnológica de Geração Solar
            </p>
          </div>
          <div className="text-left md:text-right">
            <p className="text-sm text-slate-500 print:text-slate-500 uppercase tracking-widest font-semibold mb-1">
              Apresentado a
            </p>
            <h2 className="text-2xl font-bold text-white print:text-slate-900 uppercase">
        ...
        <section className="text-center space-y-4">
          ...
          <p className="text-lg text-slate-400 print:text-slate-600 max-w-2xl mx-auto">
            Desenhamos um sistema fotovoltaico de alta performance sob medida
            para as suas necessidades de consumo, garantindo independência
            energética e retorno rápido do investimento.
          </p>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-900 border border-slate-800 print:bg-slate-50 print:border-slate-200 rounded-2xl p-6 relative overflow-hidden group">
            ...
            <p className="text-sm font-medium text-slate-400 print:text-slate-500">
              Economia Estimada (Anual)
            </p>
            <p className="text-3xl font-black text-white print:text-slate-900 mt-1">
              {formatarDinheiro(economiaAnual)}
            </p>
            <p className="text-xs text-emerald-500 font-semibold mt-2 bg-emerald-500/10 w-fit px-2 py-1 rounded">
          ...
          <div className="bg-slate-900 border border-slate-800 print:bg-slate-50 print:border-slate-200 rounded-2xl p-6 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-5">
              <LineChart className="w-24 h-24 text-orange-500" />
            </div>
            ...
            <p className="text-3xl font-black text-white print:text-slate-900 mt-1">
              {paybackAnos.toFixed(1)}{" "}
              <span className="text-lg text-slate-500 font-medium">anos</span>
            </p>
            <p className="text-xs text-orange-500 font-semibold mt-2 bg-orange-500/10 w-fit px-2 py-1 rounded">
              Payback Rápido
            </p>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 print:bg-slate-50 print:border-slate-200 rounded-2xl p-6 relative overflow-hidden">
            ...
            <p className="text-3xl font-black text-white print:text-slate-900 mt-1">
              ...
              <span className="text-lg text-slate-500 font-medium">kWh</span>
            </p>
            <p className="text-xs text-blue-500 font-semibold mt-2 bg-blue-500/10 w-fit px-2 py-1 rounded">
              Potência de {kwp} kWp
            </p>
          </div>
        </section>
          ...
          <div className="bg-slate-900 border border-slate-800 print:bg-white print:border-slate-300 rounded-2xl overflow-hidden divide-y divide-slate-800 print:divide-slate-200">
            ...
            <div className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="bg-slate-800 print:bg-slate-100 p-3 rounded-xl">
                  <SunMedium className="w-6 h-6 text-orange-400" />
                </div>
                <div>
                  <p className="text-sm text-slate-400 print:text-slate-500 font-medium uppercase tracking-wider">
                    Módulos Fotovoltaicos
                  </p>
                  <p className="text-lg font-bold text-white print:text-slate-900">
                    {projeto.modulo_modelo || "Equipamento de Alta Eficiência"}
                  </p>
                </div>
              </div>
              {/* MELHORIA-002: exibir Potência + Quantidade */}
              <div className="flex gap-8">
                <div className="text-left md:text-right">
                  <p className="text-sm text-slate-400 print:text-slate-500">
                    Potência
                  </p>
                  <p className="text-xl font-bold text-white print:text-slate-900">
                    {projeto.modulo_potencia_w || "-"} W
                  </p>
                </div>
                <div className="text-left md:text-right">
                  <p className="text-sm text-slate-400 print:text-slate-500">
            ...
            <div className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-white print:text-slate-900">
                    {projeto.inversor_modelo || "Inversor Inteligente"}
                  </p>
                </div>
              </div>
              {/* MELHORIA-002: exibir Potência + Quantidade */}
            ...
            {projeto.bateria_modelo && (
              <div className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-emerald-950/20 print:bg-emerald-50">
                <div className="flex items-center gap-4">
                  <div className="bg-emerald-900/50 print:bg-emerald-200 p-3 rounded-xl">
                    <Battery className="w-6 h-6 text-emerald-400 print:text-emerald-600" />
                  </div>
                  <div>
                    <p className="text-sm text-emerald-500 print:text-emerald-600 font-medium uppercase tracking-wider">
                      Armazenamento (BESS)
                    </p>
                    <p className="text-lg font-bold text-white print:text-slate-900">
                      {projeto.bateria_modelo}
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-800 print:border-slate-300">
          <div className="flex gap-4">
            ...
            <div>
              <h4 className="text-lg font-bold text-white print:text-slate-900">
                Garantia de Performance
              </h4>
              <p className="text-sm text-slate-400 print:text-slate-600 mt-1">
                Os painéis solares desta proposta possuem garantia de fabricação
                padrão de 12 a 15 anos e garantia de geração de energia de até
                25 anos.
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 print:text-slate-600 mt-1">
                Ao aprovar este projeto, você evitará a emissão de{" "}
                <strong>{co2Evitado} toneladas de CO2</strong> na atmosfera, o
                equivalente a plantar <strong>{arvoresSalvas} árvores</strong>.
              </p>
            </div>
          </div>
        </section>
        ...
        <footer className="pt-12 pb-8 text-center border-t border-slate-800 print:border-slate-300">
          <p className="text-lg font-bold text-white print:text-slate-900 mb-2">
          ...
          <p className="text-sm font-medium text-slate-400 print:text-slate-600 uppercase tracking-widest">
            {projeto.cliente_nome}
          </p>
        </footer>
      </main>
