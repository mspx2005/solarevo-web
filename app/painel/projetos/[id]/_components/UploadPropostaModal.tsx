...
import { uploadProposta } from "../actions";
...
export function UploadPropostaModal({
  projetoId,
  onClose,
}: {
  projetoId: string;
  ...
  async function enviar() {
    ...
    // FASE 3E.1 — ETAPA 02: integração real com a Server Action.
    const fd = new FormData();
    fd.append("projetoId", projetoId);
    fd.append("file", arquivo);

    const res = await uploadProposta(fd);

    if (res.ok) {
      setEstado("sucesso");
      // Mensagens diferenciadas conforme resultado da IA
      if (res.iaOk) {
        setMensagem("Proposta enviada e analisada com sucesso.");
        setAvisoIa(
          "Os equipamentos foram extraídos e salvos no projeto automaticamente."
        );
      } else {
      ...
      setEstado("erro");
      setMensagem(res.error ?? "Erro ao enviar a proposta.");
    }
  }
            ...
            <>
              ...
              >
                ...
                <p className="mt-3 text-sm font-medium text-enertrack-white">
                  Arraste a proposta aqui ou clique para selecionar
                </p>
